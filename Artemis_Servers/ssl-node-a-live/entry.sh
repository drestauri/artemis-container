...
# Change this to something relevant to your app to
# distinguish between any other apps logging to
# the same locaiton
APP_NAME="SSL enabled Artemis server"
...
# IMPORTANT: don't attach to stdout.log as that would cause an infinite loop of outputs!
...
# some random PID you grabbed from ps -e. This is achieved through the
# exec included in the Dockerfile (see CMD toward the bottom)
wait $pid
