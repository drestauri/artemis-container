...
pre_execution_handler() {
  ...
  echo "Started app" >> /app/stdout.log
}
...
post_execution_handler() {
  ...
  echo "Stopping app" >> /app/stdout.log
  /home/artemis/mybroker/bin/artemis-service stop
  echo "App stopped" >> /app/stdout.log
}
...
sigterm_handler() { 
  if [ $pid -ne 0 ]; then
    ...
    echo "SIGTERM handler" >> /app/stdout.log
    kill -15 "$pid"
    wait "$pid"
    post_execution_handler
  fi
...
sigquit_handler() {
echo "ERROR: Called SIGQUIT handler instead of SIGTERM. Forwarding..." >> /app/stdout.log
sigterm_handler
}

sigint_handler() {
echo "ERROR: Called SIGING handler instead of SIGTERM. Forwarding..." >> /app/stdout.log
sigterm_handler
}
...
echo "cleaning up" >>/app/stdout.log
ps -e >> /app/stdout.log
